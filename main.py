...
# Variables
date_format = '%d/%m/%Y'
start_date_string = "25/06/2021"
end_date_string = "01/07/2021"
...
unwanted_transactions = ignore_transation_file.readlines()

for transaction in unwanted_transactions:
    stripped_transaction = (transaction.strip())
    if stripped_transaction[:3] == "tx_":
        print(stripped_transaction)
        trimmed_df = trimmed_df[trimmed_df["Transaction ID"]
...
for date in list_of_date_range:
    ...
    daily_expenses_sums.append(date_sum)
    print((date, date_sum))

daily_expenses_df = pd.DataFrame(
    {'Date': daily_dates,
     'Amount': daily_expenses_sums,
     })
...
daily_average_spend = round(daily_expenses_df["Amount"].mean(), 2)

print("\n\n\n")
print("The average daily spend between {} and {} is £{}".format(
    start_date_string, end_date_string, daily_average_spend))
print("\n\n\n")
